...
function CardCom({ onClick, handleAddToWishlist, userAd, ad }) {
        ...
        <>
            {
                userAd ?
                    <Card className={isFeatured ? "" : ""} style={{ width: '18rem', minHeight: '18rem' }} onClick={onClick}>
                        ...
                        <Card.Body className={isFeatured ? "p-0 ps-2 bg-warning rounded-bottom-1" : "p-0"}>
                            <div className={isFeatured ? "d-flex flex-column gap-1 bg-white p-3 pt-1 pb-1" : "d-flex flex-column gap-1 p-3 pt-1 pb-1"} style={{ borderRadius: "0px 0px 0.2rem 0.2rem" }}>
                                ...
                                <Card.Text className='p-0 m-0 fw-normal'>{userAd.year} - {userAd.kmDriven} km</Card.Text>
                    ...
                    <Card className={isFeatured ? "" : ""} style={{ width: '18rem', minHeight: '18rem' }} onClick={onClick}>
                        <div className={isFeatured ? "imgContainer d-flex align-items-center justify-content-center" : ""} style={{ height: "11.6rem", width: "17.9rem" }}>
                            <Card.Img className={isFeatured ? "cardImg p-2" : "p-2"} style={{ height: "100%", width: "100%", objectFit: "contain" }} src={`${baseURL}${ad.imagePath[0]}`} alt='img' />
                            {
                                isFeatured &&
                                <div className='cardFeatured bg-warning rounded-1 fw-light' style={{ fontSize: "small", padding: "1px 10px" }}>FEATURED</div>
                            }
                        ...
                        <Card.Body className={isFeatured ? "p-0 ps-2 bg-warning rounded-bottom-1" : "p-0"}>
                            <div className={isFeatured ? "d-flex flex-column gap-1 bg-white p-3 pt-1 pb-1" : "d-flex flex-column gap-1 p-3 pt-1 pb-1"} style={{ borderRadius: "0px 0px 0.2rem 0.2rem" }}>
                                <Card.Title className='p-0 m-0'>{ad.price}</Card.Title>
                                <Card.Text className='p-0 m-0 fw-normal'>{ad.year} - {ad.kmDriven} km</Card.Text>
                                <Card.Subtitle className='p-0 m-0 text-muted fw-light'>{ad.brand}, {ad.year}, {ad.fuel}</Card.Subtitle>
                                <Card.Subtitle className='p-0 m-0 text-muted fw-lighter' style={{ fontSize: "small" }} >{ad.location}</Card.Subtitle>
                            </div>
                            {/* <Button variant="primary">Go somewhere</Button> */}
                        </Card.Body>
                    </Card>
