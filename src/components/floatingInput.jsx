...
import { updateShowPassword } from '../redux/slice';

function FloatingInput({ type, label, value, onChange }) {
    const id = useId()
    const dispatch = useDispatch()
    const showPasswordVar = useSelector((state) => state.counter.showPassword)
    const passwordNotEmptyVar = useSelector((state) => state.counter.passwordNotEmpty)

    const toggleVisibility = () => {
        dispatch(updateShowPassword(!showPasswordVar))
    }

    return (
        <>
            <div className="floatContainer d-flex align-items-center">
                <input className='floatingInput border border-1 border-white rounded-2' id={id} type={type === "password" ? (showPasswordVar ? "text" : "password") : type} value={value} onChange={onChange} required />
